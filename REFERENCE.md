...
* [`DISTRO_VERSION`](env/DISTRO_VERSION) - The version of the operating system distribution. Values depend on `DISTRO`. The value of `DISTRO_VERSION` on your target systems may not be numeric. Be careful using `-gt` and related tests since the version, even when numeric, may not be an integer expression for all values of `DISTRO` (e.g. OpenBSD 7.1).
...
* [`FEATURE_FILE`](option/FEATURE_FILE) - Value is a path to a file. Default to a file in `/tmp`. Set this option to control where the feature file, which is used to keep track of the progress of the Platform setup, is written. The file is deleted after the process finishes. The user must have access to write to this file.
...
* [`VAR_FILE`](option/VAR_FILE) - Value is a path to a file. Default to a file in `/tmp`. Set this option to control where the variables file, which is used to keep track of variables set by custom features, is stored. The file is deleted after the process finishes. The user must have access to write to this file. See [include/var_set](include/var_set) and [include/var_get](include/var_get) include functions.
